...
void *produce (void *args) {
	...
	entry->response.request.client_id = (int)args;
	buffer.produce (&buffer, entry);
	zlog_debug (category, "Producing %d by %u", args, pthread_self ());
	free (entry);
}
...
void *consume (void *args) {
	...
	for (count = 0; count < 100; count++) {
		...
		zlog_debug (category, "Consumed %d", entry->response.request.client_id);
		if (entry != 0)
			free (entry);
	}
}
