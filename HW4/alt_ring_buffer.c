...
int _produce (buffer_t *buffer, buffer_entry_t *buffer_entry) {
	...
	int status = simple_mutex_lock (&buffer->mutex);
	assert (status == 0);
	...
	while (buffer->count >= buffer->consume_threshold) {  
	    ...
	    status = simple_cond_wait (&buffer->cond_not_full, &buffer->mutex);  
	    assert (status == 0);
	...
	if (buffer->count == buffer->consume_threshold) {
	    ...
	    status = simple_cond_broadcast (&buffer->cond_not_empty);  
	    assert (status == 0);
	}  
...
buffer_entry_t* _consume (buffer_t *buffer) {
	...
	while (buffer->count <= buffer->produce_threshold) {  
	    ...
	    status = simple_cond_wait (&buffer->cond_not_empty, &buffer->mutex);  
	    assert (status == 0);
	...
	if (buffer->count == buffer->produce_threshold) {  
	    ...
	    status = simple_cond_broadcast (&buffer->cond_not_full);  
	    assert (status == 0);
	}  
